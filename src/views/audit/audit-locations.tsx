...
const AuditLocations: React.FC<AuditLocationsProps> = ({ auditGeneral, audits, missingRecords }) => {
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = audits.filter((audit) => {
      ...
      const locationStr = audit.location ? `${audit.location.buildingName} ${audit.location.room}`.toLowerCase() : '';
  ...
  const filteredMissing = missingRecords.filter((rec) => {
    ...
    return chemicalName.includes(missingSearch.toLowerCase()) || cas.includes(missingSearch.toLowerCase());
  });

  const currentMissing = filteredMissing.slice(missingPage * missingRowsPerPage, missingPage * missingRowsPerPage + missingRowsPerPage);
